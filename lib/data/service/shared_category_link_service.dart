...
class SharedCategoryLinkService {
  ...
  ///웹 방문자 - shareId로 링크 정보 가져오기
  Future<SharedCategoryLinkModel?> getSharedLink(String shareId) async {
    try {
      final doc = await _db.collection('sharedLinks').doc(shareId).get();
      if (doc.exists && doc.data() != null) {
        return SharedCategoryLinkModel.fromMap(doc.id, doc.data()!);
      }
    } catch (e) {
      print('SharedLink 조회 에러: $e');
    }
    return null;
  }
}